# Developer Guide: Sovereign Unity Barn Twin
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/unity-barn-twin.git
cd unity-barn-twin
...
```bash
export SBB_UNITY_WS_PORT=your_value_here
export SBB_SENSOR_MQTT_TOPIC=your_value_here
export SBB_TWIN_SCENE_ID=your_value_here
export SBB_RECORD_ENABLED=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-unity-barn-twin)
...
```bash
# Build
docker build -t sovereign-unity-barn-twin:dev .

# Run with env file
docker run --env-file .env -p 8809:8809 sovereign-unity-barn-twin:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-unity-barn-twin
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8809`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-unity-barn-twin",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8809` in use | `lsof -i :8809` | Kill PID or change `SERVICE_PORT` |
